classdef DES_Optimized < handle
    % DES_Optimized - Wersja ULTRA (SP-Box + Permutation Lookups)
    % Optymalizacje:
    % 1. SP-Network (S-Box + P złączone)
    % 2. Permutacje IP/FP oparte na tablicach (8 x 256) zamiast bit-loop
    % 3. Uint64 arithmetic
    ...
    properties
        ...
        IP_LUT   % 8 x 256 uint64 (Szybka permutacja początkowa)
        FP_LUT   % 8 x 256 uint64 (Szybka permutacja końcowa)
    end
    ...
    methods
        ...
        function init_sp_tables(obj)
            % Prekomputacja S-Box + Permutacja P
        ...
        function init_perm_lookups(obj)
            % Tworzy tablice LUT dla IP i FP.
            % Idea: Zamiast permutować 64 bity osobno, permutujemy 8 bajtów.
            % Dla każdego bajtu (0-255) wiemy dokładnie, gdzie trafią jego bity.
            
            obj.IP_LUT = zeros(8, 256, 'uint64');
            obj.FP_LUT = zeros(8, 256, 'uint64');
            
            for byteIdx = 1:8
                % Przesunięcie, aby umieścić bajt testowy w odpowiednim miejscu uint64
                % byteIdx 1 = bity 64..57 (w FIPS MSB), byteIdx 8 = bity 8..1
                shiftAmt = (8 - byteIdx) * 8;
                
                for val = 0:255
                    % Symulujemy wejście z ustawionym tylko jednym bajtem
                    testInput = bitshift(uint64(val), shiftAmt);
                    
                    % Używamy starej (wolnej) funkcji do obliczenia gdzie trafią bity
        ...
        function cipher64 = encrypt_data(obj, plain64, key64, iv64)
            ...
            for i = 1:numBlocks
                ...
                % === OPTYMALIZACJA 1: Szybkie IP przez Lookup ===
                % Rozbijamy blok na 8 bajtów i sumujemy prekomputowane pozycje
                block = bitxor(bitxor(bitxor( ...
                    obj.IP_LUT(1, bitshift(block, -56) + 1),       ...
                    obj.IP_LUT(2, bitand(bitshift(block, -48), 255) + 1)), ...
                    bitxor( ...
                    obj.IP_LUT(3, bitand(bitshift(block, -40), 255) + 1),  ...
                    obj.IP_LUT(4, bitand(bitshift(block, -32), 255) + 1))), ...
                    bitxor(bitxor( ...
                    obj.IP_LUT(5, bitand(bitshift(block, -24), 255) + 1),  ...
                    obj.IP_LUT(6, bitand(bitshift(block, -16), 255) + 1)), ...
                    bitxor( ...
                    obj.IP_LUT(7, bitand(bitshift(block, -8),  255) + 1),  ...
                    obj.IP_LUT(8, bitand(block, 255) + 1))));
                
                L = uint32(bitshift(block, -32));
                R = uint32(bitand(block, mask32));
                
                % 16 Rund Feistela (bez zmian - już zoptymalizowane)
                for r = 1:16
                    L_next = R;
                    K_curr = subKeys(r);
                    f_val = uint32(0);
                ...
                % === OPTYMALIZACJA 2: Szybkie FP przez Lookup ===
                outBlock = bitxor(bitxor(bitxor( ...
                    obj.FP_LUT(1, bitshift(preOut, -56) + 1),       ...
                    obj.FP_LUT(2, bitand(bitshift(preOut, -48), 255) + 1)), ...
                    bitxor( ...
                    obj.FP_LUT(3, bitand(bitshift(preOut, -40), 255) + 1),  ...
                    obj.FP_LUT(4, bitand(bitshift(preOut, -32), 255) + 1))), ...
                    bitxor(bitxor( ...
                    obj.FP_LUT(5, bitand(bitshift(preOut, -24), 255) + 1),  ...
                    obj.FP_LUT(6, bitand(bitshift(preOut, -16), 255) + 1)), ...
                    bitxor( ...
                    obj.FP_LUT(7, bitand(bitshift(preOut, -8),  255) + 1),  ...
                    obj.FP_LUT(8, bitand(preOut, 255) + 1))));
        ...
        function plain64 = decrypt_data(obj, cipher64, key64, iv64)
            subKeys = obj.generate_subkeys(key64);
            numBlocks = length(cipher64);
            plain64 = zeros(size(cipher64), 'uint64');
            prevBlock = iv64;
            mask32 = uint64(4294967295);
            ...
            for i = 1:numBlocks
                ...
                block = bitxor(bitxor(bitxor( ...
                    ...
                    bitxor( ...
                    obj.IP_LUT(3, bitand(bitshift(currentCipher, -40), 255) + 1),  ...
                    obj.IP_LUT(4, bitand(bitshift(currentCipher, -32), 255) + 1))), ...
                    bitxor(bitxor( ...
                    obj.IP_LUT(5, bitand(bitshift(currentCipher, -24), 255) + 1),  ...
                    obj.IP_LUT(6, bitand(bitshift(currentCipher, -16), 255) + 1)), ...
                    bitxor( ...
                    obj.IP_LUT(7, bitand(bitshift(currentCipher, -8),  255) + 1),  ...
                    obj.IP_LUT(8, bitand(currentCipher, 255) + 1))));
                ...
                plain64(i) = bitxor(decryptedBlock, prevBlock);
                prevBlock = currentCipher;
            end
        end
        
        function subKeys = generate_subkeys(obj, key64)
            % Tutaj zostawiamy prostszą wersję, bo to wykonuje się tylko RAZ na obraz
        ...
        % Stare funkcje (wolne) używane teraz TYLKO do inicjalizacji tabel
