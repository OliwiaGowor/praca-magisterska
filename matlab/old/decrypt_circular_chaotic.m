function decrypted_img = decrypt__circular_chaotic(encrypted_img, keys_struct)
%
% == POCZĄTEK ORYGINALNEGO KODU DESZYFROWANIA ==
% Logika jest identyczna jak w Twoim skrypcie.
%
% Rozpakuj klucze i parametry (teraz jako argumenty wejściowe)
...
% Inicjalizacja pustych macierzy (jak w skrypcie)
posrow=NaN(1,rows);
poscol=NaN(1,cols*8);
...
% Iteracje kluczy (identyczne jak w skrypcie)
for i=1:49
...
% STEP 6 & 7 - Odwrotna substytucja XOR
for i=1:rows*cols
    y=mod(y+ a + b*smht(y/N,A,B,C,D),N);
...
%
% == KONIEC ORYGINALNEGO KODU DESZYFROWANIA ==
%
