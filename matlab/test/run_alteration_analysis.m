...
% Data do wyświetlenia w tytule (czytelny format)
display_date = datestr(now, 'yyyy-mm-dd HH:MM');

% --- TWOJA PALETA (15 kolorów RGB 0-1) ---
colors = [
...
if length(algos) > size(colors, 1)
    extra_needed = length(algos) - size(colors, 1);
    colors = [colors; rand(extra_needed, 3)];
end
...
for t = 1:num_attacks
    ...
    % Pętla po metrykach (osobny wykres dla każdej metryki)
    for m = 1:num_metrics
        ...
        % --- PODPISY I TYTUŁ ---
        % Tytuł: Atak | Metryka | Data
        title_str = sprintf('%s | %s\nData: %s', pretty_att, pretty_metric, display_date);
        ...
        if ~isempty(plot_handles)
            ...
            try set(lgd, 'NumColumns', 4); catch; end % Dopasowanie kolumn legendy
        end

        % --- ZAPIS PLIKU ---
        % Nazwa pliku: Metric_Attack_Date.png
        ...
        close(f); % Zamknij, aby zwolnić pamięć
    end
end
end
