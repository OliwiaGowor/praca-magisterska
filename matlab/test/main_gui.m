function main_gui()
    ...
    function run_benchmark_callback(~, ~)
        ...
        try
            % 1. Przygotuj dane (używa nowego setup_test_data z img_flat)
            data = setup_test_data(current_img);
            
            % 2. Konfiguracja (wszystkie algorytmy dostępne w run_benchmarks)
            % Pobieramy listę z głównego pliku konfiguracyjnego jeśli istnieje, lub definiujemy tu
            config.titles = { ...
                'AES-CBC (Hardware-C)', 'Blowfish-CBC (Software-C)', 'ChaCha20 (Software-C)', ...
                'Chaotyczny (MATLAB)', 'DES-CBC (MATLAB)', 'AES-CBC (Pure MATLAB)', ...
                'Blowfish-CBC (Pure MATLAB)', 'ChaCha20 (Pure MATLAB)', ...
                '3D-Logistic-Chirikov (MATLAB)', 'Chaos Bit Shuffling (MATLAB)', ...
                'Hyperchaotic 2D (MATLAB)', 'Hu & Tian - Two-Stage Logistic (MATLAB)', ...
                'Entropy Wavelet-Chaos (Pourasad et al.)'};
            config.N_RUNS = 1;
            
            % 3. Uruchom
            raw = run_benchmarks(data, config);
            avg = process_results(raw, data, config);
            visualize_results(avg, raw, data, config);
